...
async fn get_groups(app: tauri::AppHandle, query: Option<String>) -> Result<Vec<GroupSummary>, String> {
    ...
    let sql = if q.is_empty() {
        "SELECT 
            g.id, g.google_id, 
            COALESCE(g.name, (SELECT u.name FROM users u JOIN group_memberships gm ON u.id = gm.user_id WHERE gm.group_id = g.id AND u.is_main_user = 0 LIMIT 1)) as resolved_name,
            g.type, 
            (SELECT COUNT(*) FROM messages WHERE group_id = g.id) as msg_count,
            g.last_message_at
         FROM groups g
         ORDER BY g.last_message_at DESC NULLS LAST".to_string()
    } else {
        format!("SELECT 
            g.id, g.google_id, 
            COALESCE(g.name, (SELECT u.name FROM users u JOIN group_memberships gm ON u.id = gm.user_id WHERE gm.group_id = g.id AND u.is_main_user = 0 LIMIT 1)) as resolved_name,
         ...
         FROM groups g
         WHERE g.id IN (
            SELECT group_id FROM group_memberships gm JOIN users u ON gm.user_id = u.id 
            WHERE u.name LIKE '%{}%' OR u.email LIKE '%{}%'
         ) OR g.name LIKE '%{}%'
         ORDER BY g.last_message_at DESC NULLS LAST", q, q, q)
    };
    ...
    let rows = stmt.query_map([], |row| {
        ...
        Ok(GroupSummary {
            ...
            last_message_at: last_msg_raw.map(|s| format_to_pst(&s)),
        })
...
async fn get_messages(app: tauri::AppHandle, group_id: i64, limit: i64, offset: i64, query: Option<String>) -> Result<Vec<MessageEntry>, String> {
    ...
    let (sql, params_vec) = if q.is_empty() {
        ("SELECT m.id, u.name, u.email, m.text, m.created_at, m.topic_id
          ...
          LIMIT ?2 OFFSET ?3", vec![group_id.to_string(), limit.to_string(), offset.to_string()])
    } else {
        ("SELECT m.id, u.name, u.email, m.text, m.created_at, m.topic_id
          FROM messages m
          JOIN users u ON m.user_id = u.id
          WHERE m.group_id = ?1 AND m.text LIKE ?2
          ORDER BY m.created_at DESC
          LIMIT ?3 OFFSET ?4", vec![group_id.to_string(), format!("%{}%", q), limit.to_string(), offset.to_string()])
    };
    ...
    let rows = stmt.query_map(rusqlite::params_from_iter(params_vec), |row| {
        ...
        let att_rows = att_stmt.query_map([msg_id], |att_row| {
            ...
            let final_path = if is_copied == 1 && managed_path.exists() { managed_path } else {
                Path::new(&takeout_path).join("Groups").join(format!("{} {}", if google_id.len() > 11 { "Space" } else { "DM" }, google_id)).join(&export_name)
            };

            Ok(AttachmentEntry {
                id: att_row.get(0)?,
                original_name: att_row.get(1)?,
                export_name,
        ...
        Ok(MessageEntry {
            ...
            text: row.get(3)?,
            created_at: format_to_pst(&created_at_raw),
            topic_id: row.get(5)?,
            attachments,
        })
    }).map_err(|e| e.to_string())?;
    ...
    tauri::async_runtime::spawn(async move {
        let _ = sync_media_for_group(app_clone, group_id).await;
    });
